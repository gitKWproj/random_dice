...
class MainDialog(QDialog):
    ...
    def setupUI(self):
        ...
        #
        # self.main_text.setPlainText("가나다라마사바아자차카타파하하하")
    ...
    def dice(self):
        ...
        if idx == 0:
            ...
            self.main_text.setPlainText("{}의 주사위 {} !!!".format(self.game.users[idx].name, result).center(32, " "))
            self.player1_text.append(
                "{}만큼 이동!! 현재 위치는 land[{}]입니다.\n남은 생명력 {}".format(result, self.game.users[idx].land_idx,
                                                                  self.game.users[idx].life))
            ...
            # 비어있는 땅에 주인값 넣기
            # self.game.total_land[self.game.users[idx].land_idx] = self.game.users[idx].name
            # realtime_land = self.game.total_land
            # self.land_text.setPlainText("{}".format(realtime_land))
            ##################################
            self.main_text.setPlainText("{}의 주사위 {} !!!".format(self.game.users[idx].name, result).center(34, " "))
            self.player2_text.append(
                "{}만큼 이동!! 현재 위치는 land[{}]입니다.\n남은 생명력 {}".format(result, self.game.users[idx].land_idx,
                                                                  self.game.users[idx].life))
